...
async function seed() {
  ...
  for (const item of contentItems) {
    const existing = await db.select().from(pageContents)
      .where(and(eq(pageContents.page, item.page), eq(pageContents.sectionKey, item.sectionKey)));
    if (existing.length === 0) {
      await db.insert(pageContents).values(item);
      console.log(`  Seeded: ${item.page}/${item.sectionKey}`);
    }
  }

  const settingsItems = [
    { key: "site_name", value: "Alliance Street Accounting Pvt Ltd" },
    ...
    { key: "meta_title", value: "Alliance Street Accounting Pvt Ltd | Global Accounting & Virtual CFO" },
